<template>
    <div class="labels-wrapper" v-click-outside="hide">
        <div class="toolbar" @click="show">
            <div class="title label-required">Labels</div>
            <div class="icon">
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            search: '',
            labels: [
                {name: 'task', label: "Task", class: 'bg-success text-white'},
                {name: 'bug', label: "Bug", class: 'bg-danger text-white'},
                {name: 'hot_fix', label: "Hot Fix", class: 'bg-danger text-white'}
            ],
        }),
        computed: {
            labelsFiltered() {
